...
export default async function handler(
  ...
  try {
    ...
    if (allNews.length > 0) {
      ...
      try {
        ...
        if (!apiKey) {
            ...
            // FIX: Return 500 status if API key is missing
            ...
            if (response.ok && result.candidates && result.candidates.length > 0 &&
              ...
              // FIX: Return 500 status if Gemini API call fails or returns invalid structure
              return res.status(500).json({ error: "Could not generate today's market summary from AI." });
            }
        }
        ...
        // FIX: Return 500 status for any unhandled errors during AI generation
        return res.status(500).json({ error: "Failed to generate market summary due to an internal AI error." });
      }
    } else {
    ...
    // FIX: Return 500 status for any unhandled errors during RSS fetching
    return res.status(500).json({ error: "Failed to retrieve market data for summary generation." });
  }

  // Only send 200 OK if summary was successfully generated or no news was available
